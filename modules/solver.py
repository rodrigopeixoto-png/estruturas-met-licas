...
class MotorCalculo3D:
    def __init__(self):
        ...
        Módulo de Elasticidade (E) e Cisalhamento (G) para Aço NBR 8800.
        """
        self.E = 200e6      # 200 GPa = 200.000.000 kN/m²
        self.G = 77e6       # 77 GPa = 77.000.000 kN/m²
        self.A = 0.005      # Área da Seção (m²) ~ Profilada/I
    ...
    def _matriz_elemento_3d(self, no1, no2):
        ...
        # Flexão Z (Cortante Y)
        12EIz_L3 = (12 * self.E * self.Iz) / (L**3)
        6EIz_L2 = (6 * self.E * self.Iz) / (L**2)
        4EIz_L = (4 * self.E * self.Iz) / L
        2EIz_L = (2 * self.E * self.Iz) / L
        
        k_loc[1, 1] = k_loc[7, 7] = 12EIz_L3
        k_loc[1, 7] = k_loc[7, 1] = -12EIz_L3
        k_loc[1, 5] = k_loc[5, 1] = k_loc[1, 11] = k_loc[11, 1] = 6EIz_L2
        k_loc[5, 7] = k_loc[7, 5] = -6EIz_L2
        k_loc[7, 11] = k_loc[11, 7] = -6EIz_L2
        k_loc[5, 5] = k_loc[11, 11] = 4EIz_L
        k_loc[5, 11] = k_loc[11, 5] = 2EIz_L

        # Flexão Y (Cortante Z)
        12EIy_L3 = (12 * self.E * self.Iy) / (L**3)
        6EIy_L2 = (6 * self.E * self.Iy) / (L**2)
        4EIy_L = (4 * self.E * self.Iy) / L
        2EIy_L = (2 * self.E * self.Iy) / L

        k_loc[2, 2] = k_loc[8, 8] = 12EIy_L3
        k_loc[2, 8] = k_loc[8, 2] = -12EIy_L3
        k_loc[2, 4] = k_loc[4, 2] = k_loc[2, 10] = k_loc[10, 2] = -6EIy_L2
        k_loc[4, 8] = k_loc[8, 4] = 6EIy_L2
        k_loc[8, 10] = k_loc[10, 8] = 6EIy_L2
        k_loc[4, 4] = k_loc[10, 10] = 4EIy_L
        k_loc[4, 10] = k_loc[10, 4] = 2EIy_L

        # Matriz de Rotação 3D (3x3)
        D = np.sqrt(cx**2 + cy**2)
        if D < 1e-5: # Elemento Vertical
        ...
        # Matriz de Transformação Global (12x12)
        T = np.zeros((12, 12))
        for b in range(4):
            T[b*3:(b+1)*3, b*3:(b+1)*3] = r3x3
            
        # K_global = T_transposta * K_local * T
        k_glob = T.T @ k_loc @ T
        return k_glob, L
    ...
    def aplicar_carga_distribuida(self, q_kNm2, vao_x, espacamento):
        ...
        for n1, n2 in self.barras:
            ...
            if (z1 + z2) / 2.0 > 0.1: # Barra de cobertura
                x1, y1, _ = self.nos[n1]
                x2, y2, _ = self.nos[n2]
    ...
    def resolver(self):
        ...
        try:
            ...
            # 1. Montagem da Matriz de Rigidez Global
            comprimentos = []
            for n1, n2 in self.barras:
                k_elem, L = self.matriz_elemento_3d(n1, n2)
            ...
            # 2. Aplicação das Condições de Contorno (Graus Livres)
            ...
            # 4. Cálculo dos Esforços Solicitantes Máximos
            desloc_max_mm = np.max(np.abs(U_completo)) * 1000.0
            
            # Estimativa conservadora dos esforços com base nos deslocamentos e cargas nodais
    ...
    def matriz_elemento_3d(self, n1, n2):
        return self._matriz_elemento_3d(n1, n2)
